...
void sendErrors( struct Errors* e)
{
	...
	servAddr.sin_port = htons(atoi("1234")); // CnC Server Port Number
	...
	printf("메시지를 입력하세요.\n");
	//gets_s(buf, MAX);
	send(hSocket, (char*)&e, sizeof(Errors), 0);
	retval = send(hSocket, (char*)& len, sizeof(int), 0);
	if (retval == SOCKET_ERROR) {
		printf("error!\n");
		exit(1);
	}

	retval = send(hSocket, (char*)& e, sizeof(Errors), 0);
	if (retval == SOCKET_ERROR) {
		printf("error!\n");
		exit(1);
	}
